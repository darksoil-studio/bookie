...
export class EditBooking extends LitElement {
  ...
  async updateBooking(fields: any) {  
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('booking-updated', {
        composed: true,
        bubbles: true,
        detail: {
    ...
    return html`
      <sl-card>
        ...
        <sl-input name="title" .label=${msg("Title")}  required .defaultValue=${ this.currentRecord.entry.title }></sl-input>          </div>
          ...
          <div style="margin-bottom: 16px">
        <sl-input name="start_time" .label=${msg("Start Time")} type="datetime-local" @click=${e => e.preventDefault()}  required .defaultValue=${new Date(this.currentRecord.entry.start_time/1000).toLocaleString()} ></sl-input>          </div>

          <div style="margin-bottom: 16px">
        <sl-input name="end_time" .label=${msg("End Time")} type="datetime-local" @click=${e => e.preventDefault()}  required .defaultValue=${new Date(this.currentRecord.entry.end_time/1000).toLocaleString()} ></sl-input>          </div>

  ...
  renderDetail(entryRecord: EntryRecord<Booking>) {
    return html`
      <sl-card>
          ...
          <sl-icon-button style="margin-left: 8px" .src=${wrapPathInSvg(mdiDelete)} @click=${() => this.deleteBooking()}></sl-icon-button>
        </div>
  ...
  render() {
    switch (this._booking.value.status) {
      ...
      case "complete":
        ...
        if (!booking) return html`<span>${msg("The requested booking doesn't exist")}</span>`;
    
        if (this._editing) {
    	  return html`<edit-booking
    	    .currentRecord=${ booking }
            @booking-updated=${async () => { this._editing = false; } }

  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No booking requests found for this resource")}</span>
      </div>`;

    return html`
      <div style="display: flex; flex-direction: column">
        ${hashes.map(hash =>
          html`<booking-request-summary .bookingRequestHash=${hash}></booking-request-summary>`
        )}
      </div>
    `;
  }
